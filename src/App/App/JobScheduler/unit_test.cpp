...
TEST_CASE("Running", "[JobScheduler]")
{
    ...
    SECTION("Simple")
    {
        for (unsigned i {0}; i < JOB_COUNT; ++i)
        {
            ...
            // are done. Thus, the test waits a few miliseconds to let the job thread do its checks.
    ...
    // Even though all the jobs have finished, the state cannot be imediatly updated, as this
