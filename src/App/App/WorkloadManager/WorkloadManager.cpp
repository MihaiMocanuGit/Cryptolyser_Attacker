...
namespace App
{
WorkloadManager::WorkloadManager(const std::atomic_bool &continueRunning)
    : m_g_continueRunning {continueRunning}
{
}

WorkloadManager::States WorkloadManager::state() const noexcept { return m_currentState.load(); }

size_t WorkloadManager::currentJobIndex() const noexcept { return m_currentJobIndex.load(); }

size_t WorkloadManager::size() const noexcept
{
    std::lock_guard lock {m_workloadMutex};
    return m_workload.size();
}

void WorkloadManager::m_throwOnInvalidIndex(size_t index) const
{
    if (index >= size())
        throw std::out_of_range("WorkloadManager: invalid job index access.");
}

bool WorkloadManager::addJob(std::unique_ptr<JobI> job)
{
...
bool WorkloadManager::removeJob(size_t jobIndex)
{
...
bool WorkloadManager::removeAllPossibleJobs()
{
    switch (m_currentState)
    {
        case States::NOT_STARTED:
...
bool WorkloadManager::swapJobs(size_t jobIndex1, size_t jobIndex2)
{
...
bool WorkloadManager::start(size_t firstJobIndex)
{
    ...
    switch (m_currentState)
    {
        case States::NOT_STARTED:
        {
            ...
            m_worker = std::jthread {&WorkloadManager::m_processWorkload, this};
            return true;
        }
        case States::BUSY:
            [[fallthrough]];
...
void WorkloadManager::m_processWorkload()
{
...
bool WorkloadManager::pauseAfterJob()
{
    switch (m_currentState)
    {
        case States::BUSY:
...
bool WorkloadManager::forceStopCurrent()
{
    switch (m_currentState)
    {
        case States::BUSY:
...
bool WorkloadManager::rearmManager()
{
    switch (m_currentState)
    {
        case States::FINISHED:
...
std::vector<std::unique_ptr<JobI>> WorkloadManager::cloneJobs() const
{
...
std::vector<std::string> WorkloadManager::jobDescriptions() const
{
...
[[nodiscard]] const std::atomic_bool &WorkloadManager::continueRunning() const noexcept
{
    return m_g_continueRunning;
}

WorkloadManager::~WorkloadManager() { m_currentState = States::FORCEFULLY_STOPPED; }
