...
struct StudyDoppelganger::StudyContext
{
    struct State
    {
    ...
    const size_t APPROX_TOTAL_COUNT{StudyDoppelganger::AES_BLOCK_SIZE *
                                    StudyDoppelganger::SAMPLE_GROUP_SIZE * AVG_SAMPLE_SIZE};
    struct Display
    {
        ...
        Display(const StudyDoppelganger::DisplayParams &display)
    ...
    struct Boundary
    {
        ...
        Boundary(const StudyDoppelganger::TimingBoundaryParams &bounds)
            : Boundary{bounds.lb, bounds.ub}
        {
        }
    } bd;

    StudyContext(size_t avgSampleSize, const StudyDoppelganger::DisplayParams &display,
                 const StudyDoppelganger::TimingBoundaryParams &bounds)
        : AVG_SAMPLE_SIZE{avgSampleSize}, display{display}, bd{bounds}
    {
    }
};

size_t StudyDoppelganger::m_totalCount(const StudyContext &ctx) const
{
...
bool StudyDoppelganger::filterCurrentValue(StudyContext &ctx, double timing)
{
...
void StudyDoppelganger::printStats(StudyContext &ctx)
{
...
{
    for (unsigned byteValue{0}; byteValue < StudyDoppelganger::SAMPLE_GROUP_SIZE; ++byteValue)
    {
        std::ifstream in;
...
void StudyDoppelganger::saveDataRaw(const std::string &directory,
...
void StudyDoppelganger::saveDataMetrics(const std::string &directory,
                                        const std::vector<SampleGroup<double>> &data)
{
    std::filesystem::create_directory(directory);
    std::vector<std::thread> threads;
...
void StudyDoppelganger::loadRawStudyData(const std::string &rawDataDir)
{
    std::vector<std::thread> threads;
    threads.reserve(AES_BLOCK_SIZE);
    for (unsigned byteBlock{0}; byteBlock < AES_BLOCK_SIZE; ++byteBlock)
...
bool StudyDoppelganger::isSaveTime(const StudyContext &ctx) const
{
...
StudyDoppelganger::StudyDoppelganger(ServerKeyConnection &&connection,
                                     volatile const int &continueRunningFlag,
...
void StudyDoppelganger::start(size_t desiredAvgSampleSize, const DisplayParams &displayParams,
    ...
    if (m_connection.connect())
    {
             ...
             ++state.currentCount)
        {
            ...
            if (isSaveTime(ctx))
            {
                ...
                StudyDoppelganger::saveDataMetrics(saveDir, m_sampleGroups);
            }

            std::vector<std::byte> studyPlaintext = m_constructRandomVector(m_DATA_PACKET_LENGTH);
            // experimenting with fixed key that's manually modified.
            std::array<std::byte, PACKET_KEY_BYTE_SIZE> studyKey{};
            studyKey.fill(static_cast<std::byte>(0));
            const auto result{m_connection.transmit(state.currentId, studyKey, studyPlaintext)};
            if (not result)
            {
        ...
        StudyDoppelganger::saveDataRaw(display.SAVE_FOLDER_PATH, m_sampleGroups);
    }
}
const std::vector<SampleGroup<double>> &StudyDoppelganger::data() const { return m_sampleGroups; }

StudyDoppelganger::TimingBoundaryParams StudyDoppelganger::calibrate(const std::string &saveTo,
                                                                     size_t transmissionsCount,
                                                                     double confidenceLB,
                                                                     double confidenceUB)
{
    if (m_connection.connect())
    {
             ...
             ++currentCount)
        {
            ...
            {
                studyKey[i] = static_cast<std::byte>(i);
            }
            const auto result{m_connection.transmit(-1, studyKey, studyPlaintext)};
            if (not result)
            {
